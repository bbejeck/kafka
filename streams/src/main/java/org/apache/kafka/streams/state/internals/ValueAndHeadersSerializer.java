...
/**
 * Serializer for ValueAndHeaders.
 ...
 * This is used by KIP-1271 to serialize values with headers for session state stores.
 */
public class ValueAndHeadersSerializer<V> implements WrappingNullableSerializer<ValueAndHeaders<V>, Void, V> {
    public final Serializer<V> valueSerializer;
    private final HeadersSerializer headersSerializer;

    ValueAndHeadersSerializer(final Serializer<V> valueSerializer) {
    ...
    @Override
    public byte[] serialize(final String topic, final ValueAndHeaders<V> valueAndHeaders) {
        if (valueAndHeaders == null) {
            return null;
        }
        return serialize(topic, valueAndHeaders.value(), valueAndHeaders.headers());
    }
    ...
    private byte[] serialize(final String topic, final V plainValue, final Headers headers) {
        ...
        try (final ByteArrayOutputStream baos = new ByteArrayOutputStream();
            ...
            throw new SerializationException("Failed to serialize ValueAndHeaders", e);
        }
    }
    ...
    public void setIfUnset(final SerdeGetter getter) {
        // ValueAndHeadersSerializer never wraps a null serializer (or configure would throw),
